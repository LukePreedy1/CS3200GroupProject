DROP DATABASE IF EXISTS imdb_solo_project;

CREATE DATABASE imdb_solo_project;

USE imdb_solo_project;
...
DROP TABLE IF EXISTS movie_person_relationship;

CREATE TABLE movie_person_relationship (
	movie_id VARCHAR(7) NOT NULL,
    person_id VARCHAR(7) NOT NULL,
    person_role ENUM('actor', 'writer', 'director') NOT NULL,
    CONSTRAINT movie_person_relationship_pk
		PRIMARY KEY (movie_id, person_id, person_role),
	CONSTRAINT movie_id_fk0
		FOREIGN KEY (movie_id)
        REFERENCES movie (movie_id),
	CONSTRAINT movie_person_relationship_person_id_fk
		FOREIGN KEY (person_id)
        REFERENCES person (person_id)
);
